...
// 初始化
function init() {
    // connectWebSocket(); // Disabled for GitHub Pages demo
    setupEventListeners();
    setupTheme();
    setupDemoMode();
}
...
function setupDemoMode() {
    // 设置演示在线人数
    elements.onlineCount.textContent = Math.floor(Math.random() * 20) + 5;
    
    // 模拟在线人数变化
    setInterval(() => {
        const change = Math.random() > 0.5 ? 1 : -1;
        const current = parseInt(elements.onlineCount.textContent);
    ...
    const demoSendMessage = () => {
        ...
        const nickname = elements.nicknameInput.value.trim() || '访客';
        ...
        // 显示演示弹幕
        showDemoDanmaku(message);
    };
    
    // 覆盖发送消息函数
    window.sendMessage = demoSendMessage;
}

// 演示弹幕
function showDemoDanmaku(text) {
    if (!elements.danmakuContainer) return;
